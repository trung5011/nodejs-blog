...
var groupsValidator   = require(__path.__path_validates+'groups');
...
var link = `/${systemConfig.prefixAdmin}/groups/list`;
var folderView = __path.__path_views+'page/backend/groups' ;

var pageTitleIndex 	= 'Groups Management - List';
...
router.post('/delete', function(req, res, next) {
	let id 				= req.body.cid;
	GroupsModel.deleteItem(id,{task:'delete-multi'}).then((result) =>{
	...
	if (errors) {
		...
		GroupsModel.saveItem(item,{task:taskCurrent}).then((result) =>{
			if(taskCurrent == "add"){
				...
				UsersModel.saveItem(item,{task:'change-group-name'}).then((results) =>{
					req.flash('success', message);
					res.redirect(link);
				});
			}
